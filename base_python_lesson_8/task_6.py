...
class Storage:
    class FullStorageError(Exception):
        ...
        def text(self):
            return self.__text
    class EmptyPlaceError(Exception):
        def __init__(self, text):
            self.__text = text
    ...
    def take_item(self, item_id):
        ...
        if item is None:
            raise self.EmptyPlaceError(f"({ item_id }) Место то - пустое X(")
        return item

    def send(self, item_id, to):
        """Отправить вещь в to"""
        if self.storage[item_id] is None:
            raise self.EmptyPlaceError(f"({ item_id }) Тут нечего отправлять. Расходимся.")
